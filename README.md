...
```bash
fly apps create starcitizen-social
...
```bash
fly apps create starcitizen-social-redis
...
```bash
fly pg create --region sjc --name starcitizen-social-db
fly pg attach starcitizen-social-db
...
3. Then, create DNS records for your custom domain.
    ...
    ```bash
    @   ALIAS starcitizen-social.fly.dev
    www CNAME starcitizen-social.fly.dev
    ```
...
Useful resources for operating and debugging a running instance include `fly logs`, `fly scale show`, `fly ssh console`, the Metrics section of `fly dashboard`, and the Sidekiq dashboard at <https://starcitizen-social.fly.dev/sidekiq> (you have to be logged in to Mastodon as an admin user to see it).
...
```bash
fly apps create starcitizen-social-predeploy
